...
function init(zip_code) {
	...
	if ($('#searchResults').length) {
		...
		$('#searchResults').children().children().each(function() {
			var price__fraction = $(this).find('span.price__fraction').text();
			var price__decimals = $(this).find('span.price__decimals').text();

			//usado como chave da 'listaProdutos' quando houver resposta da API
			current_itemID = this['id'].replace('PAD-', '');
			...
			if (!$(this).find('span.item-installments-interest').length) {
				//pega o valor da parcela mostrado na tela
				var valor_parcela = (parseFloat(
					$(this).find('span.item-installments-price').text().replace('R$ ', '')
				) +
					parseFloat($(this).find('sup.installment__decimals').text()) / 100).toFixed(2);
				//pega a quantidade de parcelas mostrado na tela
				var numero_parcelas = parseInt(
					$(this).find('span.item-installments-multiplier').text().replace('x', '')
				);

				//Arrumar aqui porque o valor nem sempre é de 12 parcelas
				//Principalmente para valores baixos
				var valor_total = (valor_parcela * numero_parcelas).toFixed(2);

				var valor_total_parcelado = document.createElement('span');
				valor_total_parcelado.setAttribute('id', 'valor_total' + current_itemID);
				valor_total_parcelado.style.fontSize = 'small';
				valor_total_parcelado.style.border = '0px';
				valor_total_parcelado.style.marginTop = '0px';
				valor_total_parcelado.style.display = 'inline-block';
				valor_total_parcelado.style.fontStyle = 'Helvetica';
				valor_total_parcelado.innerHTML = ' = ' + valor_total;
				$(this).find('span.item-installments-price').after(valor_total_parcelado);
			}
